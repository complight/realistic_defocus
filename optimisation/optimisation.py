...
class multiplane_hologram_optimiser():
    """
    A highly configurable class for optimising multiplane holograms.
    """
    ...
    def init_optimiser(self):
        """
        Internal function to set the optimiser.
        """
        if self.optimisation_mode == 'Stochastic Gradient Descent':
            self.optimiser = torch.optim.Adam([{'params': [self.phase, self.offset]}], lr=self.learning_rate)
        else:
            self.optimiser = torch.optim.Adam([{'params': [self.phase]}], lr=self.learning_rate)
    ...
    def optimise(self):
        """
        Function to optimise multiplane phase-only holograms.

        Returns
        -------
        hologram_phase             : torch.tensor
                                     Phase of the optimised phase-only hologram.
        reconstruction_intensities : torch.tensor
                                     Intensities of the images reconstructed at each plane with the optimised phase-only hologram.
        """
        if self.optimisation_mode == 'Stochastic Gradient Descent':
            hologram = self.stochastic_gradient_descent()
        elif self.optimisation_mode == 'Gerchberg-Saxton':
            hologram = self.gerchberg_saxton()
        elif self.optimisation_mode == 'Double Phase':
    ...
    def gerchberg_saxton(self):
        """
        Function to optimise multiplane phase-only holograms using Gerchberg-Saxton algorithm.
    ...
    def stochastic_gradient_descent(self, delta=0.0):
        """
        Function to optimise multiplane phase-only holograms using stochastic gradient descent.
        ...
        for step in t:
            for plane_id in range(self.number_of_planes):
                ...
                if self.image_location == 0: 
                   ...
                   phase_only = torch.zeros_like(self.phase)
                   ...
                   phase = torch.nan_to_num(phase_only, nan=2*np.pi)
                else:
                   phase = self.phase
                hologram = generate_complex_field(self.amplitude, phase)
                distances = self.set_distances(plane_id)
